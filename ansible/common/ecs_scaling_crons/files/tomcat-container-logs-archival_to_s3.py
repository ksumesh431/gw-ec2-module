#!/usr/bin/python
import os, subprocess, re, requests, shutil, socket, boto3
...
# processed_logs_path = "/root/processed_logs/"
processed_logs_path = "/opt/teams/share/root_logs/"

# container logs path from where logs will be copied
container_logs_path = "/opt/teams/share/" + environment + "/logs/"
container_logs_path_archive = "/opt/teams/share/" + environment + "/logs/archive"
...
starting_day_of_log_archival = (
    "Saturday"  # Case sensitive format like "Monday", "Tuesday"
)
...
slack_hook_url = (
    "https://hooks.slack.com/services/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"
)


def get_hostname():
    hostname = socket.gethostname()
    print("The script is running on: ", hostname)
    return hostname
...
def datewise_search_and_push(path_to_search, path_to_push, date_to_search):
    if (
        ...
        if not matching_files:
            print("No logs matched for today\n\n")
...
def datewise_search_and_push_archive(path_to_search, path_to_push, date_to_search):
    if (
        ...
        if not matching_files:
            print("No archived logs matched for today\n\n")
...
def get_bucket_path(pattern):
    s3 = boto3.client("s3")
    response = s3.list_buckets()
    buckets = [bucket["Name"] for bucket in response["Buckets"]]
    regex = re.compile(pattern)
    bucket_dict = [bucket for bucket in buckets if regex.search(bucket)]
...
def upload_file_to_s3(file_name, bucket, object_name):
    s3_path_to_move = (
        object_name.rstrip("/").split("/")[-1]
        + "/"
        + file_name.rstrip("/").split("/")[-1]
    )
    compress_file_path = "/root/processed_logs/" + file_name.rstrip("/").split("/")[-1]
    delete_compress = ["rm", "-rf", compress_file_path]
    try:
        session = boto3.Session(profile_name="default")
        s3_client = session.client("s3")
        s3_client.upload_file(file_name, bucket, s3_path_to_move)
        print(
            f"S3 Move for  '{file_name}' to s3 bucket '{bucket}' to '{s3_path_to_move}' is completed"
        )
        subprocess.run(delete_compress, check=True)
...
def compress_directory(path_to_archive, archive_destination, s3_path, bucket_name):
    ...
    deletion_path = processed_logs_path + base_dir
    tar_command = [
        "tar",
        "-czf",
        archive_path,
    ...
    delete_directory = ["rm", "-rf", deletion_path]
    try:
        # Run the tar command
        subprocess.run(tar_command, check=True)
        print(f"Directory '{path_to_archive}' compressed to '{archive_path}'\n")
        upload_file_to_s3(archive_path, bucket_name, s3_path)

        print(f"Removing '{base_dir}'")
        subprocess.run(delete_directory, check=True)
        print(f"Log Rotation completed and '{base_dir}' should be removed")
...
def main():
    try:
        ...
        bucket_pattern = r"pd-log-archive"
        bucket_name = get_bucket_path(bucket_pattern)
        s3_path = "s3://" + bucket_name + "/" + bucket_name + "-after-13-May-2024/"
        print(" S3 bucket path logs are pushed to: ", s3_path)
        ...
        ####################################################################################################################
        ####################################################################################################################
        ################################## Compressing processed logs and push to s3 #######################################
        ####################################################################################################################
        if current_date.strftime("%A") == starting_day_of_log_archival:
            print(
            ...
            compress_directory(
                previous_main_directory_path, processed_logs_path, s3_path, bucket_name
            )
        ####################################################################################################################
        ################################## Compressing processed logs and push to s3 #######################################
        ####################################################################################################################
